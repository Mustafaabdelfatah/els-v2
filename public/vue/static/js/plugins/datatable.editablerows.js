...
class EditableRows {
  ...
  _createInstance() {
    ...
    this._datatable = jQuery('#datatableRows').DataTable({
      ...
      pageLength: 10,
      columns: [{ data: 'User Name' }, { data: 'Average to close' }, { data: 'Closed Requests' }],
      language: {
        paginate: {
  ...
  _addListeners() {
    ...
    if (document.getElementById('ConfirmButton'))
      document.getElementById('ConfirmButton').addEventListener('click', this._addEditFromModalClick.bind(this));
    ...
    if (document.getElementById('addEditModal'))

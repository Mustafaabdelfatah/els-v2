...
class DatatableExtend {
  ...
  _addListeners() {
    ...
    document.querySelectorAll('.datatable-filter .dropdown-item').forEach((el) => {
      el.addEventListener('click', this._onFilterChange.bind(this));
    });
    ...
    // });
    document.querySelector(".export-excel").addEventListener('click', this._onExportClick.bind(this))
  ...
  _onFilterChange(event) {
    ...
    this._getDatatable(event.currentTarget.closest('.datatable-filter')).search(value).draw();
  ...
  _onExportClick(event) {
    console.log(event.currentTarget.classList, 'asdsa')
    event.preventDefault();

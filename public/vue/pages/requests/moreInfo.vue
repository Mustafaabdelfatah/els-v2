  ...
  <div class="modal-dialog" style="max-width: 50%">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-outline-primary" data-bs-dismiss="modal" @click="cancel">{{ $t('Cancel')
        }}</button>
...
export default {
  ...
  methods: {
    ...
    approvedForm() {
      ...
      window.jQueryStartLoading();
      let formData = new FormData
      for (let key in this.approveForm.files) {
        console.log("loop", this.approveForm.files[key]);
        const value = this.approveForm.files[key];
        if (value != 'undefined' || value != null)
          formData.append('files[' + key + ']', value);
      }
      ...
      formData.append('informationText', this.form.informationText);
    ...
    rejected() {
      ...
      if (this.errors.length <= 0) {
        ...
        formData.append('comment', this.reject_comment);
        formData.append('form_id', this.id);
        formData.append('fileName', this.fileName);
        this.$axios({
          url: 'admin/forms/return_request',
          ...
          console.log('response is :', response.data, response.data.type)
          if (response.data.type == "reject"){
      ...
      if (filesArr.length > 0) {
        for (let i = 0; i < filesArr.length; i++) {
          ...
          reader.onload = function () {
            ...
            // self.approveForm.files = reader.result;
            self.approveForm.files.push(reader.result); // Append file to the array

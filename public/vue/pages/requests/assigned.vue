            ...
            <div class="row">
              ...
              <div class="col-sm-12 col-md-7 col-lg-9 col-xxl-10 text-end mb-1">
                <div class="d-inline-block me-0 me-sm-3 float-start float-md-none">
                  ...
                  <button class="btn btn-icon btn-icon-only btn-foreground-alternate shadow need-info-datatable disabled"
                    data-bs-delay="0" data-bs-toggle="tooltip" data-bs-placement="top" :title="$t('Need_Info')"
                    type="button" @click="needInfo">
                    <i data-cs-icon="content"></i>
                  </button>
                  <button
                    class="btn btn-icon btn-icon-only btn-foreground-alternate shadow close-request-datatable disabled"
                    data-bs-delay="0" data-bs-toggle="tooltip" data-bs-placement="top" :title="$t('close_Request')"
                    type="button" @click="endRequest">
                    <i data-cs-icon="check"></i>
                  </button>
                  <!-- Review Button End -->

                  <!-- Approve Secret Button Start -->
                  <button class="btn btn-foreground-alternate shadow disabled approve-secret-datatable" data-bs-delay="0"
                    data-bs-toggle="tooltip" data-bs-placement="top" :title="$t('Approve if secret data sent to you')"
                    type="button" @click="approveSecret">
                    {{ $t('Approve_Secret') }}
                    <!--<i data-cs-icon="rotate-left"></i>-->
                  </button>
                  <!-- Approve Secret Button End -->
                ...
                <div class="d-inline-block">
                  ...
                  <div class="d-inline-block shadow">
                    <select class="form-select" id="status" v-model="status" @change="filterDataTable">
                  ...
                  <div class="d-inline-block shadow">
                    <select class="form-select" id="user_id" v-model="user_id" @change="filterDataTable">
                      <option value="">{{ $t("All_Employees") }}</option>
                      <option v-for="(employee, index) in employees" :key="index" :value="employee.id">{{ employee.name }}
                      </option>
                    </select>
                  </div>
...
export default {
  ...
  created() {
    ...
    window.datatableServerSideUrl =
      this.$axios.defaults.baseURL + "admin/assigned-forms";
    ...
    window.datatableServerSideColumnDefs = [
      ...
      },
      {
        ...
        render: (data, type, row, meta) => {
          if (row.amendments.length > 0) {
            if (row.amendments[0].is_secret != 'undefined' && row.amendments[0].is_secret == 1) {
              for (let i = 0; i < row.amendments.length; i++) {
                return (row.amendments[0].is_secret == 1 && row.amendments[0].approve_secret == 0 && row.form.status == 'returned') ? `<span data-tooltip="` + this.$i18n.t('please approve if data sent') + `" data-tooltip-position="top" style="margin: 20px;" class='badge rounded-pill bg-danger'>No</span>`
      ...
      },
      {
        ...
        render: (data, type, row, meta) => {
          ...
          else if (row.form.status == "returned")
      ...
      },
      {
        ...
        render: function (data, type, row, meta) {
          ...
          //   '<div><i class="text-center d-inline-block text-primary icon-20 cs-more-horizontal"></i></div>';
          if (row.amendments.length > 0)
            for (let i = 0; i < row.amendments.length; i++) {
              if (row.amendments[i]) {
      ...
      },
      {
        targets: 16, // check
        render: function (data, type, row, meta) {
          return (row.amendments.length > 0 && (row.amendments[0].status == 1 || (row.amendments[0].is_secret == 1 && row.amendments[0].approve_secret == 0))) ?
            row.amendments[0].created_at :
            '<div><i class="text-center d-inline-block text-primary icon-20 cs-more-horizontal"></i></div>';
        }
      },
      {
